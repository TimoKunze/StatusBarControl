...
BOOL IsValidPanelIndex(int panelIndex, StatusBar* pStatBar)
{
	...
	HWND hWndStatBar = NULL;
	if(pStatBar) {
		OLE_HANDLE h = NULL;
		pStatBar->get_hWnd(&h);
		hWndStatBar = static_cast<HWND>(LongToHandle(h));
	}

	return IsValidPanelIndex(panelIndex, hWndStatBar);
}

BOOL IsValidPanelIndex(int panelIndex, HWND hWndStatBar)
{
	if(panelIndex < 0 || !IsWindow(hWndStatBar)) {
		return FALSE;
	}

	return (panelIndex < static_cast<int>(SendMessage(hWndStatBar, SB_GETPARTS, 0, 0)));
}
